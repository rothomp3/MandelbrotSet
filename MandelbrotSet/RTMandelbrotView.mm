...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        [self setColors:[[RTColorTable alloc] initWithColors:500]];
...
- (void)drawRect:(CGRect)dirtyRect
{
    ...
    CGFloat pixelSize = 0.5;
    if ([UIScreen mainScreen].scale == 1.0) // nonretina
        pixelSize = 1.0;
    ...
    for (CGFloat i = 0; i < bounds.size.width; i+=pixelSize)
    {
        ...
        {            
            ...
            {
                double vz = ([mandelbrot escapedAt] - log2(log2([mandelbrot zabs]))) * 23.4;
                unsigned int colorIndex = (int)(vz) % [colors numColors];
